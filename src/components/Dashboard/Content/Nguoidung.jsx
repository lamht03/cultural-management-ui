...
const Nguoidung = () => {
  ...
  const [permissionErrorShown, setPermissionErrorShown] = useState(false); // New state for permission error
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axiosInstance.get(`/v1/HeThongCanBo/DanhSachCanBo`, {
        params: {
          TenCanBoOrTenNguoiDung: searchName,
          CoQuanID: selectedCoQuanID,
          pageNumber: 1,
          pageSize: 30
        }
      });
  ...
  const postData = async (requestData) => {
    try {
      ...
      message.error('Bạn không có quyền để thực hiện thao tác này');
    }
  };
